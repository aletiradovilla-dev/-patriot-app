     ...
     <ScrollView contentContainerStyle={styles.content}>
       ...
       {tramos.map((tramo, i) => (
         <View key={i} style={styles.tramoBox}>
           ...
           <TouchableOpacity style={styles.dateInput} onPress={() => setActivePicker(activePicker === i ? null : i)}>
       ...
       <View style={styles.paxContainer}>
         <TextInput
           ...
           value={pax}
           onChangeText={(v) => setPax(v.replace(/[^0-9]/g, ''))}
           keyboardType="number-pad"
         />
         <TouchableOpacity
           style={styles.paxBtn}
           onPress={() => setPax(String(Math.max(1, parseInt(pax || '1') - 1)))}
         >
           <Text style={styles.paxBtnText}>−</Text>
         </TouchableOpacity>
         <TouchableOpacity
           style={[styles.paxBtn, { marginRight: 8 }]}
           onPress={() => setPax(String(Math.min(20, parseInt(pax || '1') + 1)))}
         >
           <Text style={styles.paxBtnText}>+</Text>
         </TouchableOpacity>
       </View>
...
paxContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.05)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', borderRadius: 12 },
paxInput: { flex: 1, padding: 16, color: '#fff', fontSize: 14 },
paxBtn: { width: 36, height: 36, borderRadius: 8, borderWidth: 1, borderColor: 'rgba(255,255,255,0.12)', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,255,255,0.04)', marginRight: 6 },
paxBtnText: { color: 'rgba(255,255,255,0.35)', fontSize: 20, fontWeight: '300', lineHeight: 22, textAlign: 'center' },
